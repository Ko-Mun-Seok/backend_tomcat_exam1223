...
public class BoardService {
	...
	public List<Board> getList(){
		return repository.findAll();  // select * from board;
	}
	
	// 3. 게시글 작성 기능(파일업로드 포함/로컬저장)
	// - 파일업로드 기능을 구현할 때, 하나의 Entity로 처리 X
	// - 요청정보롤 담는 별도의 DTO 클래스를 만들어서 처리
	// - entity는 DB와 연동되는 모델 유지
	// - Entity 구조의 노출 위험
	// - 파일 저장 정책(NCP Object Storage, AWS S3)이 변경 되었을 때 Entity까지 영향
//	public void register(BoardRequestDTO dto) throws Exception {
//		
//		// 1. 로컬위치에 저장하는 로직 구현
//		String savedPath = null;  // 업로드ㅡ 후 저장될 경로
